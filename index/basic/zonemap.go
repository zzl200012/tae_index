...
func (zm *ZoneMap) BatchUpdate(vec *vector.Vector) error {
	if !zm.typ.Eq(vec.Typ) {
		return mock.ErrTypeMismatch
	}
	zm.mu.Lock()
	defer zm.mu.Unlock()
	if err := mock.ProcessVector(vec, zm.UpdateLocked, nil); err != nil {
		return err
	}
	return nil
}
...
func (zm *ZoneMap) Query(key interface{}) (int, error) {
	...
	max := zm.GetMaxLocked()
	min := zm.GetMinLocked()
	gt := mock.Compare(key, max, zm.typ) > 0
	lt := mock.Compare(key, min, zm.typ) < 0
	if gt {
		return 1, nil
	}
	if lt {
...
func (zm *ZoneMap) MayContainsAnyKeys(keys *vector.Vector) (bool, *roaring.Bitmap, error) {
	...
	if err := mock.ProcessVector(keys, process, nil); err != nil {
