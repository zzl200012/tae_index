...
func TestZoneMapNumeric(t *testing.T) {
	...
	rows := 1000
	vec := mock.MockVec(typ, rows, 0)
	err = zm.BatchUpdate(vec)
	require.NoError(t, err)
	...
	vec = mock.MockVec(typ1, rows, 2000)
	err = zm.BatchUpdate(vec)
	require.Error(t, err)
